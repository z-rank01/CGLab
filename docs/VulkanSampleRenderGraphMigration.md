...
应用通过窄 `engine::vulkan::render_program` 选择 pass 名称和基础 raster 配置；当前 VulkanSample 使用 `DrawPass`，TriangleSample 使用 `TrianglePass`。pass 读取 arena/uniform buffer，将 acquired swapchain image 作为 imported color attachment，把 RG transient depth image 作为 depth attachment。
...
- VulkanSample、TriangleSample 与 Render Graph targets 的 Debug/Release 编译。
