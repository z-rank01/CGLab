...
> v5（2026-08）：将原 `src/framework/` 合并进 `src/engine/`，统一 engine runtime 的目录、命名空间与构建目标。
...
- [应用层分离与共享 Engine Runtime](ApplicationRuntime.md)：应用组合点、共享 runner、sample/runtime 服务和新增 App 流程。
...
依赖只能从应用指向 engine runtime 和 renderer，再指向 engine core。engine 公共接口不得出现 Vulkan、SDL 或 glTF 类型。

## 分层

> v2 改为四层模型：在原三层草图基础上拆出**引擎运行时层**。
> 原因：UI/窗口/IO/帧循环是所有 sample 共享的运行时代码，不属于任何一个 app；
> 若留在应用层，每写一个 sample 就要复制一遍（当前 app_sample 的实际问题）。

**上层 - 应用层（samples）**：每个实验是独立 executable，例如 GltfSponzaSample、TriangleSample，以及未来的 Hi-Z/Ray Tracing sample。应用只提供元数据、启动场景、Vulkan render program 和未来的专属 UI hook，不拥有窗口、帧循环、camera、scene、控制平面或 loader worker。详见 [ApplicationRuntime.md](ApplicationRuntime.md)。

**引擎运行时层（engine runtime）**：被所有 app 复用的应用主流程——
- 帧循环与渲染窗口（当前是 SDL 方案）
...
**已解决**（不要重复劳动，也不要推翻）：
- Runtime 复用：`cglab_engine_runtime` 已拥有窗口、显式帧 phase、camera/scene、控制平面与 asset service；应用不再复制帧循环
...
- 构建边界：已有 `cglab_engine_core`、`cglab_platform_sdl`、`cglab_asset_gltf`、`cglab_asset_runtime`、`cglab_engine_runtime`、`cglab_vulkan_backend`、`cglab_application_runner`
...
- 资源服务：engine runtime 只依赖 `asset_service` 接口；`cglab_asset_runtime` 是默认实现，测试可注入 fake service
...
- Vulkan backend 已成为独立 target 和 backend façade；context/swapchain/frame/geometry/graph 已拆成 backend 内部源文件，不影响 engine runtime API
...
- `engine::vulkan::render_program` 是 Vulkan sample 的扩展点，可以逐步暴露 Render Graph/Vulkan 能力，但不得反向泄漏到 engine runtime 或 engine core。
...
**4. 引擎 UI 与单窗口（engine runtime 稳定后，详见 InterfaceRedesign.md v2）**
- 正式 UI 面板（资产、场景、rg debug 图示等），需要保证延展能力和健壮性，类似 Unity 可拼接拉伸的界面。
- 渲染窗口已可用且简单；单窗口集成（B1/B2）在 engine runtime 稳定后进行。
...
- `src/engine/`：应用主流程 runtime、sample/runtime services、asset service interface，以及 geometry 和 render backend 等中立核心接口；内部按 `cglab_engine_runtime` 与 `cglab_engine_core` 两个 target 保持依赖和测试边界；
