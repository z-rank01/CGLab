...
// - 输出：按 mesh 展平的 primitive 布局行（对齐后字节偏移、源 blob 元素偏移、draw 语义字段）
//   与每 mesh 图元计数（句柄边界）；任何越界/容量耗尽返回结构化 error。
...
namespace engine
{
    // 每 primitive 的 arena 布局计划
    struct primitive_plan
    {
        ...
        std::uint32_t first_index = 0;           // draw：index 单位
        std::int32_t vertex_offset = 0;          // draw：vertex 单位
        std::uint32_t material_index = 0;        // material_base + primitive.material（无材质回退 base）
    };

    struct geometry_upload_plan
    {
        std::vector<primitive_plan> primitives;         // 按 mesh 顺序展平
        std::vector<std::uint32_t> mesh_primitive_counts; // 每 mesh 图元数（句柄边界）
        std::uint64_t cursor = 0;                       // 计划末尾 arena 游标（成功时）
        std::string error;                              // 非空 = 失败（范围越界/容量耗尽）
        [[nodiscard]] explicit operator bool() const noexcept { return error.empty(); }
    };

    // 为 [first_mesh, first_mesh + mesh_count) 的 mesh 行段生成布局计划。
    // material_base 由材质上传结果传入；capacity 为 arena 容量，cursor 为当前游标。
    [[nodiscard]] inline geometry_upload_plan plan_geometry_uploads(
        ...
        std::uint64_t cursor)
    {
        ...
        for (std::uint32_t m = 0; m < mesh_count; ++m)
        {
            ...
            for (std::uint32_t r = 0; r < mesh.primitive_count; ++r)
            {
                ...
                {
                    plan.error = "Primitive slice out of blob bounds";
                    return plan;
                }
                ...
                {
                    plan.error = "Geometry arena exhausted";
                    return plan;
                }
                ...
                row.first_index = static_cast<std::uint32_t>(row.index_byte_offset / sizeof(std::uint32_t));
                row.vertex_offset = static_cast<std::int32_t>(row.vertex_byte_offset / sizeof(vertex));
                row.material_index = primitive.material == invalid_asset_index ? material_base
                                                                               : material_base + primitive.material;
                plan.primitives.push_back(row);
